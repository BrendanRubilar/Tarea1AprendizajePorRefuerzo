...
class GridWorld(gym.Env):
    ...
    def step(self, action):
        ...
        if self.state in self.goals or np.random.rand() < self.fail_rate:
            ...
            #Problema 2, añadir acciones estocasticas
            r = np.random.rand()
            if r < 0.7:
                actual_action = action
            elif r < 0.85:
                actual_action = (action - 1) % 4  # izquierda relativa
            else:
                actual_action = (action + 1) % 4  # derecha relativa
    ...
    def render(self, mode='human', close=False):
            ...
            if mode == 'human':
                ...
                img = Image.fromarray(grid).resize((self.n * 50, self.m * 50), Image.NEAREST)
                big_grid = np.array(img)
                self.viewer.imshow(big_grid)
